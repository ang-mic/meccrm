...
import io.meccrm.application.http.{RestInterface2, RestInterface, ServerComponent}
import io.meccrm.domain.customer.{CustomerRes, CustomerService}
...
trait ComponentRegistry extends ServerComponent with ConfigComponent {
  ...
  val customerResource = new CustomerRes(customerService)
  val webInterface = new RestInterface2(customerResource)
